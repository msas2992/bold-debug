...
class UbxStream(object):
            ...
            if self._dev.in_waiting > 0:
                if counter < 2:
                    ...
                    if self._version == 3:
                        ...
                        print('ubx class/id:', ubx_class, ubx_id)
                    else:
    ...
    def getPortConfig(self):
        msg = UbxMessage('06', '00', msg_type='tx')
        print(msg.msg)
    ...
    def enable_NMEA(self):
        ...
        for i in ids1:
            val = False
            while(val == False):
                val = self.enable_message(classes[0], i)
                if(val):
                    string += "Class: {} ID: {} // ".format(classes[0], i)
                    counter += 1
                    break
        ...
        for i in ids2:
            val = False
            while(val == False):
                val = self.enable_message(classes[1], i)
                if(val):
                    string += "Class: {} ID: {} // ".format(classes[1], i)
                    counter += 1
                    break
...
class UbxMessage(object):
    def __init__(self, ubx_class, ubx_id, msg_type="rx", **kwargs):
        print(msg_type, ubx_class, ubx_id)
        ...
        if(msg_type == "rx"):
            ...
            if(ubx_class == '01'):
                ...
                if ubx_id in message:
                    message[ubx_id]()
                else:
                    print('ubx_id', ubx_id, 'not known')
            ...
            #CFG
                ...
                message = {'zzzz00': lambda: self.__ubx_CFG_PRT(kwargs["rate"]),
...
if __name__ == '__main__':
    ser = serial.Serial('COM31', baudrate=115200, timeout=1)
    x = UbxStream(ser)

    # x.enable_message(0xF0, 0x03)
    # x.enable_NMEA()    
